...
public class KafkaExporter {
    ...
    private static void setupTronContext(ExporterConfig config) {
        ...
        if (Args.getInstance().getStorage() != null) {
            ...
            // Force Usage of RocksDB Engine for Read-Only Access
            // This is critical because RocksDB supports 'openReadOnly', which guarantees NO compaction or writes.
            // LevelDB (org.iq80) does not support strict read-only and will trigger compaction writes.
            Args.getInstance().getStorage().setDbEngine("ROCKSDB");
            
            // Set RocksDB Cache Size (64GB) via System Property (handled by modified RocksDbSettings)
            long totalCacheSize = 64L * 1024 * 1024 * 1024;
            System.setProperty("rocksdb.block.cache.size", String.valueOf(totalCacheSize));
            
            log.info("Forced DB Engine to ROCKSDB for strict Read-Only access.");
            log.info("Set RocksDB Block Cache to {} GB", totalCacheSize / (1024 * 1024 * 1024));
